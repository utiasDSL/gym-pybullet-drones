...
def run(
    ...
    # if vision:
    #     if num_drones > 1:
    #         depth_fig, depth_axs = plt.subplots(num_drones, 1, figsize=(10, 5 * num_drones))
    #         occ_fig = plt.figure(figsize=(10, 5 * num_drones))
    #         occ_axs = [occ_fig.add_subplot(num_drones, 1, i + 1, projection='3d') for i in range(num_drones)]
    #     else:
    #         #depth_fig, depth_ax = plt.subplots(1, 1, figsize=(10, 5))
    #         occ_fig = plt.figure(figsize=(10, 5))
    #         occ_ax = occ_fig.add_subplot(111, projection='3d')
    # plt.ion()
    ...
    for i in range(0, int(duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        #                 # depth_axs[j].axis('off')

        #                 ax = occ_axs[j]
        #             else:
        #                 # depth_ax.imshow(obs[str(j)]["dep"], cmap='plasma')
        #                 # depth_ax.set_title(f'Drone {j} Depth Image')
        #                 # depth_ax.axis('off')

        #                 ax = occ_ax

        #             ax.clear()
        #             ax.set_title(f'Drone {j} Occupancy Grid')
        #             ax.set_xlabel('X')
        #             ax.set_ylabel('Y')
        #             ax.set_zlabel('Z')

        #             occupied_voxels = np.argwhere(occupancy_grid)
        #             ax.scatter(occupied_voxels[:, 0], occupied_voxels[:, 1], occupied_voxels[:, 2], c='blue', marker='o')
