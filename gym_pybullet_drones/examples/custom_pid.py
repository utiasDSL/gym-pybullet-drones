...
The simulation is run by a `CustomCtrlAviary` environment.
...
from gym_pybullet_drones.envs.CustomCtrlAviary import CustomCtrlAviary
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for j in range(num_drones):
            logger.log(drone=j,
                       ...
                       state=obs[j],
                       control=np.hstack([TARGET_POS[i, 0:2], INIT_XYZS[j, 2], INIT_RPYS[j, :], np.zeros(6)])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Helix flight script using CustomCtrlAviary and DSLPIDControl')
