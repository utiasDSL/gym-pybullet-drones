...
class MeetupAviary(BaseMultiagentAviary):
    ...
    def _clipAndNormalizeStateWarning(self,
        ...
        if not(clipped_ang_vel_rp == np.array(state[13:15])).all():
            print("[WARNING] it", self.step_counter, "in MeetupAviary._clipAndNormalizeState(), clipped angular velocity [{:.2f} {:.2f}]".format(state[13], state[14]))
        if not(clipped_ang_vel_y == np.array(state[15])):
            print("[WARNING] it", self.step_counter, "in MeetupAviary._clipAndNormalizeState(), clipped angular velocity [{:.2f}]".format(state[15]))
