...
import gym_pybullet_drones.utils.position_commads as ps
...
def run(
    ...
    init_step = 0.2
    INIT_XYZS = np.array([[init_step*(i+2), init_step*(i+2), init_step*(i+2)] for i in range(num_drones)])
    # INIT_RPYS = np.array([[0, 0,  i * (np.pi/2)/num_drones] for i in range(num_drones)])
    INIT_RPYS = np.array([[0, 0,  i*0] for i in range(num_drones)])
    ...
    for i in range(NUM_WP):
        if i == 0:
            ...
            continue        
        # Good    
        # if i < NUM_WP/num_drones:
        #     TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], 0, 0, 0)
        # elif i < 2*NUM_WP/num_drones:
        #     TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], 0.03, 0.04, 0)
        # else:
        #     TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], 0, 0, 0)

        if i < NUM_WP/num_drones:
            TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], 0.001,0.001,0.001)
        elif i < 2*NUM_WP/num_drones:
            TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], -0.001, -0.001, -0.001)
        else:
            TARGET_POS[i :] = ps.relative_movement(TARGET_POS[i-1, 0], TARGET_POS[i-1, 1], TARGET_POS[i-1, 2], 0.03, 0.04, 0)
            
        print(f"{i} :: {TARGET_POS[i, :]} ")
