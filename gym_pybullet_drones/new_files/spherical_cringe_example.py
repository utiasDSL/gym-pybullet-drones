...
def run(
    ...
    Radius = 0.5
    init_const = 0.1
    INIT_XYZ = np.array([ 0, 0, init_const]).reshape(1,3)
    TARGET_POS = np.zeros((NUM_WP,3))
    for i in range(NUM_WP):
        t = 2*np.radians((2*np.pi*i)/NUM_WP)   
        TARGET_POS[i, :] = pc.spherical_to_cartesian(3*i/NUM_WP + init_const, 10*t, t*i/1000)  
    wp_counter = 0
    AGGR_PHY_STEPS = int(simulation_freq_hz/control_freq_hz) if aggregate else 1     # зачем это вообще нужно блин епрст
