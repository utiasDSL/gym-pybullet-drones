    ...
    def _nnlsRPM(self,
        ...
        if thrust < 0 or thrust > self.MAX_THRUST:
            if new_line:
                print()
                new_line = False
            print("[WARNING] ctrl it", self.control_counter, "in SimplePIDControl._nnlsRPM(), unfeasible thrust {:.2f} outside range [0, {:.2f}]".format(thrust, self.MAX_THRUST))
        if np.abs(x_torque) > self.MAX_XY_TORQUE:
            if new_line:
                print()
                new_line = False
            print("[WARNING] ctrl it", self.control_counter, "in SimplePIDControl._nnlsRPM(), unfeasible roll torque {:.2f} outside range [{:.2f}, {:.2f}]".format(x_torque, -self.MAX_XY_TORQUE, self.MAX_XY_TORQUE))
        if np.abs(y_torque) > self.MAX_XY_TORQUE:
            if new_line:
                print()
                new_line = False
            print("[WARNING] ctrl it", self.control_counter, "in SimplePIDControl._nnlsRPM(), unfeasible pitch torque {:.2f} outside range [{:.2f}, {:.2f}]".format(y_torque, -self.MAX_XY_TORQUE, self.MAX_XY_TORQUE))
        if np.abs(z_torque) > self.MAX_Z_TORQUE:
            if new_line:
                print()
                new_line = False
            print("[WARNING] ctrl it", self.control_counter, "in SimplePIDControl._nnlsRPM(), unfeasible yaw torque {:.2f} outside range [{:.2f}, {:.2f}]".format(z_torque, -self.MAX_Z_TORQUE, self.MAX_Z_TORQUE))
        ...
        if np.min(sq_rpm) < 0:
            ...
            if new_line:
                print()
                new_line = False
            print("[WARNING] ctrl it", self.control_counter, "in SimplePIDControl._nnlsRPM(), unfeasible squared rotor speeds, using NNLS")
