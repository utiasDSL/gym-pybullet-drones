...
from rrt_python_gym.kdtree_c import KDNode, KDTree
import random
import numpy as np
import time
...
class RRT_start:
    ...
    def setPt(self, startPt, endPt, xl, xh, yl, yh, zl, zh, local_range, max_iter, sample_portion, goal_portion):
        ...
        self.rand_z = random.uniform(self.z_l + self.safety_Margin, self.z_h + self.safety_Margin)
    ...
    def tracePath(self):
        ...
        for endnode in self.endList:
            if (not self.checkValidEnd(endnode)) or (not self.checkEnd(endnode)) or (not endnode.valid):
                print(self.checkValidEnd(endnode),self.checkEnd(endnode), endnode.valid)
                continue
            else:
