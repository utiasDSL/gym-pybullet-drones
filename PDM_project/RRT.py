...
class RRTStar:
    def __init__(self, start, goal, obstacle_list, rand_area, 
        ...
        # CHANGED: Dynamic search radius logic
        # It should be at least slightly larger than expand_dis
        if search_radius is None:
            self.search_radius = self.expand_dis * 2.0 
        else:
            self.search_radius = search_radius

        self.resolution = 0.5  # INCREASED resolution for faster checking
    ...
    def find_neighbors(self, new_node):
        return [node for node in self.node_list if np.linalg.norm([node.x - new_node.x, node.y - new_node.y, node.z - new_node.z]) < self.search_radius]
    ...
    def get_nearest_node(self, node_list, rnd_node):
        dists = [np.linalg.norm([n.x - rnd_node.x, n.y - rnd_node.y, n.z - rnd_node.z]) for n in node_list]
        return node_list[np.argmin(dists)]

    def get_random_node(self):
        if random.randint(0, 100) > self.goal_sample_rate:
            return Node(random.uniform(self.min_rand, self.max_rand), random.uniform(self.min_rand, self.max_rand), random.uniform(0.1, 2.0))
